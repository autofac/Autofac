        ...
        public void Initialize(IncrementalGeneratorInitializationContext context)
        {
            ...
            IncrementalValuesProvider<INamedTypeSymbol> classDeclarations = context.SyntaxProvider
                .CreateSyntaxProvider(
                    ...
                    transform: static (ctxt, _) =>
                    {
                        var syntax = ((ClassDeclarationSyntax)ctxt.Node);

                        if (ctxt.SemanticModel.GetDeclaredSymbol(syntax) is INamedTypeSymbol symbol)
                        {
            ...
            context.RegisterSourceOutput(firstSyntax,
...
");
            for (var argCount = 1; argCount <= maxArgs; argCount++)
            {
                ...
                GenerateExtensionMethod(sb, getDelegateInvokerName, argCount, withComponentContext: true);
        ...
        private static void GenerateExtensionMethod(StringBuilder sb, Func<int, bool, string> getDelegateInvokerName, int argCount, bool withComponentContext)
        {
    ...
    public static IRegistrationBuilder<TComponent, SimpleActivatorData, SingleRegistrationStyle>
        Register<{methodGenericTypeList}> (
...
");
    ...
    public sealed class {typeName}<{GetTypeParamList(reusableStrBuilder, withComponentContext: false, argCount)}> : BaseGenericResolveDelegateInvoker");
        ...
        {
            var reusableStrBuilder = new StringBuilder();
            var delegateGenericTypeList = GetTypeParamList(reusableStrBuilder, withComponentContext: false, argCount);
